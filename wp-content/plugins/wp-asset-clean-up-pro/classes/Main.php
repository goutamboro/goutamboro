...
SQL;
		...
		add_action( 'wp_ajax_' . WPACU_PLUGIN_ID . '_check_external_urls_for_status_code',
			array( $this, 'ajaxCheckExternalUrlsForStatusCode' ) );
	}
	 ...
	 */
	public static function alterWpStylesScriptsObj()
    {
        add_action('wp_print_styles', static function() {
	        ...
	        if (! empty($wpStylesList)) {
	            foreach ($wpStylesList as $styleHandle) {
	                $styleHandleObj = apply_filters('wpacu_'.$styleHandle.'_css_handle_obj', $wp_styles->registered[$styleHandle]);
		            $wp_styles->registered[$styleHandle] = $styleHandleObj;
                }
            }
        });

        foreach (array('wp_print_scripts', 'wp_print_footer_scripts') as $actionToAdd) {
	        add_action( $actionToAdd, static function() {
		        global $wp_scripts;
		        ...
		        if ( ! empty( $wpScriptsList ) ) {
			        foreach ( $wpScriptsList as $scriptHandle ) {
				        $scriptHandleObj = apply_filters('wpacu_'.$scriptHandle.'_js_handle_obj', $wp_scripts->registered[$scriptHandle]);
				        $wp_scripts->registered[$scriptHandle] = $scriptHandleObj;
			        }
		        }
	        } );
        }
